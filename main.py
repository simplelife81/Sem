...
BOT_TOKEN = os.environ.get("BOT_TOKEN", "8512191032:AAH31mvy1j7Msr_dxo3k9kYH5EtDPbXdurc")
...
#bot = Client(
    #"bot",
    #api_id=32546882,
    #api_hash=81254ed5acbeb5839a3493a995994864,
    #bot_token=8512191032:AAH31mvy1j7Msr_dxo3k9kYH5EtDPbXdurc)
...
async def pwwp_callback(bot, callback_query):
    ...
    if user_id not in auth_users:
        await bot.send_message(callback_query.message.chat.id, f"**You Are Not Subscribed To This Bot DM for access @happyq123**")
        return
...
async def get_cpwp_course_content(
    ...
    try:
        ...
        async with session.get(content_api, headers=headers, params=params) as res:
        ...
        for content in contents:
            ...
            if not url_val:
                logging.warning(f"No URL for content: {name}")
                continue
            ...
            if "media-cdn.classplusapp.com/tencent/" in url_val:
                ...
                vid_id = url_val.split("/")[-1].split(".")[0]
                url_val = (
                    "https://media-cdn.classplusapp.com/"
                    "alisg-cdn-a.classplusapp.com/"
                    "b08bad9ff8d969639b2e43d5769342cc62b510c4345d2f7f153bec53be84fe35/"
                    f"{vid_id}/master.m3u8"
                )
                ...
                url_val.startswith("https://media-cdn.classplusapp.com")
            ...
            if url_val.endswith(("master.m3u8", "playlist.m3u8")):
                ...
                content_tasks.append(
                    asyncio.create_task(
                        process_cpwp_url(url_val, name, session, headers2)
                    )
                )
        ...
        logging.exception(f"Error in folder {folder_id}: {e}")

        if retry_count < MAX_RETRIES:
            await asyncio.sleep(2 ** retry_count)
            return await get_cpwp_course_content(
                session, headers, Batch_Token, folder_id, limit, retry_count + 1
            )
